import sys
sys.path.extend(["/home/workingage/WACode/polimi"])

import polimi.params as params
...
from polimi.pathosnet import pathosnet_multimodal_classifier, pathosnet_voice_classifier
from aud import aud_classifier
...
def audeering_classifier(audio_file_path):
...
def main(arguments):
    ...
    args_parser.add_argument('--polimi_model_weight', type=float, required=True,
                             help="Weight of the PoliMI model in the enseble.")
    args_parser.add_argument('--audeering_model_weight', type=float, required=True,
                             help="Weight of the Audeering model in the enseble.")
    # Output arguments
    ...
    if args.modality == 'multimodal':
        polimi_classifier = pathosnet_multimodal_classifier(*classifier_args, verbose=args.verbose)
    else:
        polimi_classifier = pathosnet_voice_classifier(*classifier_args, verbose=args.verbose)
    ...
    for index, row in data_df.iterrows():
        ...
        polimi_multilabel_prediction, _ = polimi_classifier(*input_args)
        aud_multilabel_prediction, _ = audeering_classifier(os.path.join(args.data_path, row[args.audio_file_path_col_name]))
        multilabel_prediction = dict([(l, (args.polimi_model_weight * polimi_multilabel_prediction[l]) +
                                       (args.audeering_model_weight * aud_multilabel_prediction[l])) for l in LABELS])
        binary_prediction = {k: sum([multilabel_prediction[l] for l in LABELS_CONVERSION_DICT[k]]) for k in BINARY_LABELS}
    ...
    with open(output_scores_path, 'w') as f:
        f.write("ENSEMBLE ------------------------------------------------------------\n\n")
        f.write("PoliMI weight: " + str(args.polimi_model_weight) + ", Audeering weight: " + str(args.audeering_model_weight))
        f.write("\n\n")
